...
namespace SimpleMvvm.Core.Navigation
{
    public class NavigationParameters : INavigationParameters, INavigationParametersInternal
    {
        ...
        private Dictionary<string, object> _external = new Dictionary<string, object>();
        ...
        {
            return _external.ContainsKey(key);
        }

        public IEnumerator<KeyValuePair<string, object>> GetEnumerator()
        {
            return _external.GetEnumerator();
        }
        ...
        {
            return _external.Where(x => x.Key == key).Select(x => (T)x.Value);
        }
